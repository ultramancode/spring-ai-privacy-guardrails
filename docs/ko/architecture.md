...
Spring AI Privacy Guardrails는 **개인정보 토큰화**로 분석기가 탐지한 원문을
요청별 **불투명 토큰(opaque token)**으로 바꿔 모델에 전달합니다. 불투명 토큰은
원문 값을 직접 드러내지 않는 대체 문자열입니다.
도구를 실행하기 직전에 정책이 허용한 원문 값만 복원하고, 실행 결과를 다시 보호합니다.
출력 보호를 활성화하면 최종 응답에도 설정한 정책을 적용합니다.
...
애플리케이션 정책에 따라 해석합니다. 불투명 토큰과 원본 값의 매핑은 요청별
...
`core` 모듈은 Spring에 의존하지 않고 탐지 결과 해석, 개인정보 토큰화, 세션 관리와 내장
...
불투명 토큰과 원본 값의 매핑은 요청별 `PrivacySession`을 통해 관리됩니다. `core`를 직접
사용할 때와 Spring AI 연동을 사용할 때 모두 같은 세션 모델을 사용합니다.

각 요청에는 별도의 세션이 만들어지고, 생성된 불투명 토큰은 해당 요청 안에서만 유효합니다.
Spring AI 실행 컨텍스트에는 세션 식별자(`handle`)만 전달하며, 실제 불투명 토큰과 원본 값의
