---
description: >-
  Inspect model-bound text with rules, an HTTP guard model, or a local ONNX model.
...
| `inspection-onnx` | Local model execution, token windows and model-specific adapters |
...
An `inspectorId` identifies a configured instance, not a model family or protocol. IDs must be unique within a service. Named HTTP factories and the named ONNX constructor allow two instances of the same implementation to have different endpoints, thresholds or application policy.
...
ONNX may split a segment into multiple token windows while retaining the same segment ID. A segment belongs in `completedSegmentIds` only when all of its windows have been inspected. ONNX-specific work is limited by `OnnxInspectionConfig.maxWindows`, across all segments in one inspection. The default is 256 windows. Configure it independently from common limits, for example `new OnnxInspectionConfig(modelPath, 2, 128)`. The named constructor `new OnnxContentInspector("local-guard", config, adapter)` sets instance identity; the unnamed constructor defaults to the adapter's `modelId()`. An inspector owns its adapter and native session and must be closed. Tokenization, tensor compatibility and score decoding remain local to the ONNX module.
