...
<!-- i18n-source-sha256: 1a8a4283d2c773eddb81e961e6bd3c71a8804cea2e9769c0be07e7c4d1461f46 -->
...
애플리케이션 정책에 따라 해석합니다. 요청별 `PrivacySession`은 토큰과 개인정보 원문
값의 매핑을 관리합니다.
...
`PrivacySession`은 한 요청 안에서 토큰과 개인정보 원문 값 사이의 매핑을 관리합니다.
`core`를 직접 사용할 때와 Spring AI 연동을 사용할 때 모두 같은 세션 모델을 사용합니다.

각 요청에는 별도의 세션이 만들어지고, 생성된 토큰은 해당 요청 안에서만 유효합니다.
Spring AI 실행 컨텍스트에는 세션 식별자(`handle`)만 전달하며, 실제 토큰과 원문 값의
매핑은 라이브러리 내부 세션에 유지됩니다. 이 매핑은 모델 요청이나 도구 입력에 포함되지
않습니다.
