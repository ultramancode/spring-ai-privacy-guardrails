...
public class PrivacySecurityAutoConfiguration {
    ...
    PrivacySecurityChatClientConfigurer privacySecurityChatClientConfigurer(
            ObjectProvider<PrivacyChatClientConfigurer> privacyConfigurerProvider,
            SpringSecurityToolBoundary boundary
    ) {
        PrivacyChatClientConfigurer privacyConfigurer = privacyConfigurerProvider.getIfAvailable();
        if (privacyConfigurer == null) {
            throw new IllegalStateException(
                    "spring.ai.privacy.security.enabled=true requires "
