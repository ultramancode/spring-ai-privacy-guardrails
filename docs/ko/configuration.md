...
<!-- i18n-source-sha256: 0286c21ff86ec6c23c2fd19a95f755dd437b4265cabf6bba2af1c4be80164b83 -->
...
| core 직접 값 트리의 문자열·숫자 콘텐츠 누계 | 1,000,000자 |
| 누적 정규 분석기 입력 | 1,000,000자 |
| 해석된 JSON/값 트리 문자열 또는 속성 이름 하나 | 250,000자 |
| JSON/값 트리 숫자 표기 하나 | 1,000자 |
| 지수를 전개한 숫자 값 하나 | 4,096자 |
| 속성 이름을 포함한 JSON/값 트리 노드 | 100,000개 |
| JSON/값 트리 중첩 단계 | 128 |
...
상한을 강제해야 합니다. 이 절대 상한은 설정으로 변경할 수 없으며, 애플리케이션은
라이브러리를 호출하기 전에 더 작은 상한을 적용할 수 있습니다. 상한을 넘으면 위임
대상 실행 또는 결과 전달 전에 `PAYLOAD_LIMIT_EXCEEDED`가 발생합니다.
...
`tokenizeValueTree`와 `detokenizeValueTree`는 `null`, 불리언, 문자열, 유한한
`Byte`, `Short`, `Integer`, `Long`, `BigInteger`, `BigDecimal`, `Float`, `Double`
값, 리스트와 문자열 키를 가진 맵을 허용합니다. 임의 객체, 배열, 그 밖의 컬렉션,
문자열이 아닌 맵 키, 유한하지 않은 숫자, 참조 순환과 위 절대 상한을 넘는 입력은
거부합니다. 순환을 만들지 않는 공유 컨테이너는 허용합니다. 두 메서드는 유효한 입력
트리의 복사본을 만든 다음 변환하고 새로운 맵과 리스트 컨테이너를 반환합니다.
지원하지 않는 구조와 순환에는 `TRANSFORMATION_CONFLICT`, 상한 위반에는
`PAYLOAD_LIMIT_EXCEEDED`가 발생합니다. 오류 메시지에는 거부한 값이나 객체 내용이
포함되지 않습니다.
POJO나 직렬화 라이브러리 전용 트리 유형은 애플리케이션이 자체 직렬화 설정을 사용해
지원되는 맵/리스트 형태로 변환해야 합니다. `core` 모듈은 이 변환을 위해 리플렉션을
사용하거나 직렬화 라이브러리에 의존하지 않습니다.
