---
description: >-
  규칙, HTTP guard 모델, 로컬 ONNX 모델로 모델에 전달할 텍스트를 검사합니다.
...
<!-- i18n-source-sha256: dbd4216c670cb19bc6e62bc90b9a4294d6a37c6c38e91990165735a76f10e68c -->
...
| `inspection-onnx` | 로컬 모델 실행, 토큰 윈도우, 모델별 어댑터 |
...
`inspectorId`는 모델 계열이나 프로토콜이 아닌, 구성된 인스턴스를 식별합니다. 서비스 안에서 ID는 고유해야 합니다. 이름을 받는 HTTP 팩토리와 ONNX 생성자를 사용하면 같은 구현의 두 인스턴스에 서로 다른 endpoint, 임계값, 애플리케이션 정책을 적용할 수 있습니다.
...
ONNX는 segment 하나를 여러 토큰 윈도우로 나눌 수 있으며, 이때도 segment ID는 유지합니다. 해당 segment의 모든 윈도우를 검사해야 `completedSegmentIds`에 포함합니다. ONNX 작업량은 한 번의 검사에 포함된 모든 segment를 합쳐 `OnnxInspectionConfig.maxWindows`로 제한합니다. 기본값은 256개 윈도우입니다. `new OnnxInspectionConfig(modelPath, 2, 128)`처럼 공통 한도와 별도로 구성합니다. `new OnnxContentInspector("local-guard", config, adapter)`로 인스턴스 ID를 지정하며, 이름을 생략한 생성자는 어댑터의 `modelId()`를 사용합니다. Inspector는 어댑터와 native 세션을 소유하므로 사용이 끝나면 닫아야 합니다. Tokenizer, tensor 호환성, 점수 계산은 ONNX 모듈이 담당합니다.
